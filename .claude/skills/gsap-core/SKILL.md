...
Use invisible utility class (e.g. `invisible`) to your elements and then use GSAP's autoAlpha property to show it (or animate it in) when the page loads. autoAlpha affects opacity and visibility, changing it to visible when the opacity is greater than 0.
...
- ✅ Use invisible utility class (e.g. `invisible`) to hide elements before animation starts and use **autoAlpha** to fade them in.
